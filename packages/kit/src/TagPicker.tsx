...
/**
 * LIFT NOTE — upstream these four strings came from `useTranslation()`, two of
 * them interpolated (`{{label}}`, `{{count}}`). The kit carries no i18n
 * runtime, so they arrive as a `labels` prop with English defaults; the two
 * interpolated ones are functions, which is what an i18n `t(key, values)` call
 * reduces to once the dictionary is gone.
 */
export interface TagPickerLabels {
  /** Placeholder of the search box, when `placeholder` is not given. */
  searchPlaceholder: string;
  /** Text of the chip that adds a free-text value. */
  create: (label: string) => string;
  /** Text of the expand toggle, given the number of hidden options. */
  showAll: (count: number) => string;
  /** Text of the collapse toggle. */
  collapse: string;
}

const DEFAULT_LABELS: TagPickerLabels = {
  searchPlaceholder: 'Start typing or pick below…',
...
interface TagPickerProps extends Partial<FieldA11y> {
  ...
  pinnedValue?: string;
  /** Overrides for the four built-in strings. Defaults are English. */
...
export function TagPicker({
  value, onChange, options, placeholder, visibleCount = 8, allowCreate = true, searchable = true, pinnedValue, disabled, labels, ...a11y
}: TagPickerProps) {
  ...
  return (
    <div>
      ...
      {!searching && (hiddenUnselected > 0 || expanded) && (
        ...
        >
          {expanded ? text.collapse : text.showAll(hiddenUnselected)}
        </button>
      )}
    </div>
  );
